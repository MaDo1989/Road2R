...
const getPatientsPhonesText = (patient) => {
  ...
  for (var i = 0; i < patient.escorts.length; i++) {
    if (
      ...
      txt +=
        `${patient.escorts[i].DisplayName}: ${patient.escorts[i].CellPhone}` +
        sep;
  }
  return txt;
};
...
function messageDate(date) {
  ...
  if (sameDay && !afternoonRide) txt = `היום בשעה ${hour}:${min}`;
    ...
    txt = `ביום ${dayInWeek} ${rideDate.getDate()}.${
      rideDate.getMonth() + 1
    } בשעה ${hour}:${min}`;
  else
    txt = `ביום ${dayInWeek} ${rideDate.getDate()}.${
...
const patientMessage = (patient) => {
  ...
  else if (numberOfEscorts === 1) txt += `עם מלווה אחד`;
  else txt += `עם ${numberOfEscorts} מלווים`;
  txt += sep;
  return txt;
};
